...
math_problems_3 = [
    ...
    },
    {
        ...
        "solution": r"\text{a) vypočítame aktualizovanú, súčasnú hodnotu peňazí pomocou diskontovania, } \\[3mm] K_0 = K_n(1+r)^{-n} \\[3mm] NPV = CF_p - N_{ip} = \sum_{t=1}^{T_ž}CF_t(1+i)^{-t} - N_{ip} \\[3mm] \text{V našom prípade neriešime } N_{ip} \\[3mm] \text{0.rok } N_{ip, 0} = -1500 * (1+0.1) ^ {-0} = -1500 \\[3mm] \text{1.rok } N_{ip, 1} = 800 * 1.1^{-1} = 727.28 \vdots",
        "table": None,
        "code": None,
        "solutiontable": {
            "a": pd.DataFrame({
