...
math_problems_3 = [
    ...
    },
    {
        ...
        "solution": r"\text{Očakávaná priemerná hodnota CF sa vypočíta ako vážený priemer jednotlivých CF} \\[3mm] \text{a váhami sú pravdepodobnosti jednotlivých možností CF.} \\[3mm] \text{Súčet váh (pravdepodobností) sa rovná jednej.} \\[3mm] \text{Pri symetrickom rozdelení pravdepodobnosti, tzv. normálnom rozdelení,} \\[3mm] \text{sa očakávané CF zhoduje so stredom tohto rozdelenia.} \\[3mm] \bar{A} = \sum_{x=1}^n A_xP_x \\[3mm] \bar{A_{A}} = 0.2 * 2000 + 0.3 * 4000 + 0.3 * 6000 + 0.2 * 8000 = 5000 \text{ tis.} \\[3mm] \bar{A_{B}} = 0.1*2000 + 0.4*4000 + 0.4*6000 + 0.1*8000 = 5000 \text{ tis.} \\[3mm] \text{Priemerná hodnota CF je v tomto príklade rovnaká pre obidva projekty} \text{a preto nemôže postačovať na rozhodnutie o výhodnejšom projekte.}",
        "table": pd.DataFrame({
            "- ": ["Projekt A", "", "Projekt B", ""],
