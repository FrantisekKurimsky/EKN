...
math_problems_2 = [
    ...
    },
    {
        ...
        "solution": r"AHOJ \textbf{Pomerné stále náklady, tzv. ročná kvóta} \\[3mm] \text{Vyjadruje prepočet stálych nákladov (investičných a prevádzkových) na jeden priemerný rok životnosti investície} \\[3mm] k = \frac{r^{T_ž} * (r-1)}{r^{T_ž}-1} + p_ú + p_{mz} + p_{ost}\text{, kde: } \\[3mm]  p_ú \text{ : pomerné náklady na údržbu,} \\[3mm]p_{mz} \text{ : pomerné náklady na mzdy,} \\[3mm] p_{ost} \text{ : ostatné stále prevádzkové náklady (dodávky, služby, réžia atď.),} \\[3mm] T_ž \text{ :  ekonomická životnosť DHM, takisto } \\[3mm] \text{pomerná anuita (umorovateľ) } a_{T_ž} = \frac{r^{T_ž} * (r-1)}{r^{T_ž}-1} \\[3mm] \text{a pomerné stále prevádzkové náklady }k'=p_ú + p_{mz} + p_{ost}. ",
        "table": None,
        "code": "5* (0.1*1.05**3 + 0.15*1.05**2 + 0.25*1.05**1 + 0.4*1.05**0 + 0.1*1.05**-1)",
        "solutiontable": None
    },
